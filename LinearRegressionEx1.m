...
%2. Calculate the regression line: WingLengthPredicted= a + b * age
%
%First calculating slope:
n=length(Age);
AgeSum=sum(Age);
AgeMean=mean(Age);
WingSum=sum(WingLength);
WingMean=mean(WingLength);

SumXY = AgeSum*WingSum;
Sumxy = SumXY-(SumXY/n);
SumX2 = (AgeSum).^2;
Sumx2 = SumX2-((SumX2)/n);
SumY2 =sum(WingLength).^2;

b=Sumxy/SumX2; %slope!


a=WingMean-b*AgeMean; %calculating intercept from equation
...
%Test null hypothesis that b(slope) = 0  - I want to go over this later!
%Uses F-statistic equation: (SSE1-SSE2/m)/(SSE2/n-k)
%compares 2 variances by dividing them
%SSE = residual sum of squares: sum of all differences between observation and
%mean squared
df=n-2;
totalSS=WingSum-SumY2/n; %totalSS is the sum of the square of the difference between (each Y and mean Y)
regressionSS=Sumxy^2/Sumx2;
residualSS=totalSS-regressionSS;
Fstat=regressionSS/(residualSS/df);
prob=fpval(Fstat,1,n-2); % significance probability for regression -- I have an error for this
